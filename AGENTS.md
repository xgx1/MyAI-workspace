...
- `dsh-extensions/` 是单检出（分支 `main`）：`plugins/`（自研，受本仓版本控制）与 `skills/` 就是生产 `link:` 的目标；`vendor/` 是**第三方上游克隆**（dsh-genui / dsh-toolkit / dsh-drop-to-path / rider-skills），各自带独立 `.git` 与远端、被本仓 `.gitignore` 忽略——改它们要在各自目录里提交。需要隔离改动时按 `~/.dsh/AGENTS.md`「如果要重新引入…自行开 worktree」的指引新建。
